...
class File_cptfTest < Test::Unit::TestCase
  
  def test_get_pathname
    assert_equal("/Users/karenhawthorn/Documents/Ruby_apps/TG43/tests",File_cptf::get_pathname(__FILE__))
  end
  
  def test_csv_write_and_read
    FCSV.open("csv_test.csv", "w") do |csv|
      csv << ["row", "of", "CSV", "data"]
      csv << ["another", 1.34]
    end
    a=[]
    FasterCSV.foreach("csv_test.csv") do |row|
      a<<row
    end
    assert_equal(["row", "of", "CSV", "data"],a[0])
    assert_equal(["another", '1.34'],a[1])
    File.delete("csv_test.csv")
  end
  
  def test_should_read_af_excel_file
a="(degrees),2.5,5,10,20,30,50
...
180,0.806,0.609,0.585,0.603,0.622,0.662"
    ...
    i=0
    app_root=File.join(File.dirname(__FILE__), '..')
    FasterCSV.foreach(app_root+"/lib/af_file.csv") do |row|
      t=b[i].split(",")
      assert_equal(t,row)
      i+=1
    end
